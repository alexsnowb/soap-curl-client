...
class SoapTest extends \Codeception\Test\Unit
{
    /**
     * @var \UnitTester
     */
    ...
    {
        $this->expectErrorMessage('Invalid login or password, please provide a correct credentials');
        $client           = new Client($_ENV['WSDL_URL']);
        $userLoginRequest = new UserLogin('wrong', 'wrong', 'wrong');
        $client->userLogin($userLoginRequest);
    }
    ...
    public function testCreatePolicy()
    {
        ...
        $this->assertTrue($createPolicyResponse->return->error->errCode == 0);
        $this->assertIsFloat($createPolicyResponse->return->price);
